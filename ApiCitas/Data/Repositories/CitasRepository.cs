...
namespace ApiCitas.Data.Repositories
{
    public class CitasRepository : ICitasRepository
    {
        ...
        public async Task<int> Create(Cita cita)
        {
            using (var connection = _context.CreateConnection())
            {
                var query = @"INSERT INTO Citas (UsuarioID, CitaFecha, CitaDescripcion, CitaLugar, CitaEstado, CitaFechaCreacion) 
                          VALUES (@UsuarioID, @CitaFecha, @CitaDescripcion, @CitaLugar, @CitaEstado, GETDATE());
        ...
        public async Task<int> Update(Cita cita)
        {
            using (var connection = _context.CreateConnection())
            {
                var query = @"UPDATE Citas 
                          SET UsuarioID = @UsuarioID, CitaFecha = @CitaFecha, CitaDescripcion = @CitaDescripcion, 
                              CitaLugar = @CitaLugar, CitaEstado = @CitaEstado, CitaFechaCreacion = GETDATE() 
                          WHERE CitaID = @CitaID";
